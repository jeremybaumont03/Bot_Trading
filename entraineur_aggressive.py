...
def envoyer_alerte_telegram(message):
    ...
    except Exception as e: print(f"⚠️ Erreur Telegram : {e}")
...
def executer_trades(portfolio, settings):
    ...
    for ticker in TICKERS:
        ...
        # ── 1. GESTION DES SORTIES (Toujours active) ──────────────────────────
        if position_ouverte:
            ...
            if vendre:
                ...
                trade = {
                    ...
                    "mise"          : pos.get('mise', 1), # ✅ OBLIGATOIRE POUR LE SORTINO v4.4
        ...
        # ── 2. GESTION DES ENTRÉES (Soumise au Panic Mode) ────────────────────
...
if __name__ == "__main__":
    print("🤖 BOT PAPER TRADING V2 - AGRESSIF (ATR HYBRIDE + VUE GLOBALE)")
    ...
    # ✅ FIX : On ne fait plus de sys.exit(0) ici ! 
    # Même si le Master Brain bloque les achats, on doit faire tourner le script pour gérer les sorties.
    ...
    if trades:
        ...
        for t in trades:
            if t['action'] == 'ACHAT':
                ...
                lignes.append(f"{emoji} VENTE {t['ticker']} — PnL : {t.get('pnl', 0):+.2f}€ ({t.get('raison', '')})")
